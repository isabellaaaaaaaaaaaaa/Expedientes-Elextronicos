...
import { ArrowLeft, Plus, Folder, ChevronRight, FolderOpen, Pencil, Check, X, Phone, Mail, Heart, Calendar, Layers, FileClock, User as UserIcon, History } from 'lucide-react';
...
import { getBitacoraByEmployee, getChangesByEmployee } from '../lib/auditLog';
...
export default function EmployeeProfile({ employeeId, user, initialYear, onNavigate }: EmployeeProfileProps) {
  ...
  const [activeTab, setActiveTab] = useState<'perfil' | 'bitacora' | 'historial'>('perfil');
  ...
  const bitacoraEntries = getBitacoraByEmployee(empExpedients.map(e => e.id));
  const changeEntries = getChangesByEmployee(empExpedients.map(e => e.id));
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center gap-1.5 bg-white border border-slate-200/60 rounded-xl p-1 shadow-sm">
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold rounded-lg transition-colors ${activeTab === 'bitacora' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}
        >
          <FileClock size={14} /> Bitácora
          {bitacoraEntries.length > 0 && (
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full tabular-nums ${activeTab === 'bitacora' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'}`}>{bitacoraEntries.length}</span>
          )}
        </button>
        <button
          onClick={() => setActiveTab('historial')}
          className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold rounded-lg transition-colors ${activeTab === 'historial' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}
        >
          <History size={14} /> Historial de cambios
          {changeEntries.length > 0 && (
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full tabular-nums ${activeTab === 'historial' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'}`}>{changeEntries.length}</span>
          )}
        </button>
      </div>
      ...
      {activeTab === 'bitacora' && (
        <div className="card overflow-hidden">
          <div className="px-5 py-3.5 border-b border-slate-100 flex items-center gap-2.5">
            ...
            <p className="text-sm font-bold text-gray-800">Bitácora del empleado</p>
            {bitacoraEntries.length > 0 && (
              <span className="text-xs bg-slate-100 text-slate-600 font-bold px-2 py-0.5 rounded-full">{bitacoraEntries.length}</span>
            )}
          </div>
          {bitacoraEntries.length === 0 ? (
            <EmptyState icon={FileClock} title="Sin actividad registrada" description="Aquí aparecerán las acciones realizadas en los expedientes del empleado." compact />
          ) : (
      ...
      {activeTab === 'historial' && (
        <div className="card overflow-hidden">
          <div className="px-5 py-3.5 border-b border-slate-100 flex items-center gap-2.5">
            ...
            {changeEntries.length > 0 && (
              <span className="text-xs bg-slate-100 text-slate-600 font-bold px-2 py-0.5 rounded-full">{changeEntries.length}</span>
            )}
          </div>
          {changeEntries.length === 0 ? (
            <EmptyState icon={History} title="Sin cambios registrados" description="Aquí aparecerán los cambios realizados en los expedientes del empleado." compact />
          ) : (
            <div className="divide-y divide-slate-50">
              {changeEntries.map(e => (
