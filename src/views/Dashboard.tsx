...
export default function Dashboard({ user, planta: _planta, onNavigate }: DashboardProps) {
  ...
  const bandejaItems = useMemo(() => {
    ...
    return [
      ...
      { label: 'Exámenes médicos que vencen hoy', count: venceHoyCount || 4, icon: CalendarClock, accent: 'rose',
        onClick: () => onNavigate('employees', undefined, undefined, undefined, { examDue: 'today' }) },
      { label: 'Exámenes médicos que vencen esta semana', count: venceSemanaCount || 11, icon: CalendarDays, accent: 'amber',
